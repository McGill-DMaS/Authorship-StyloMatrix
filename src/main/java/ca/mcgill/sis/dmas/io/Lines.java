...
public abstract class Lines implements Iterable<String> {
	...
	public static Lines fromFileFullyCached(String file, Charset charset)
			throws Exception {
	...
	public static Lines fromFileGzip(String file, Charset encoding)
			throws Exception {
	...
	public static String readAll(String file, Charset charset, boolean isGzip)
			throws Exception {
	...
	public static ArrayList<String> readAllAsArray(String file,
			Charset charset, boolean isGizp) throws Exception {
		file = DmasApplication.applyDataContext(file);
		Lines lines;
		if (!isGizp)
	 ...
	 * @return
	 */
	public static ArrayList<String> selectAsList(Lines lines,
			int start_included, int end_excluded) {
		ArrayList<String> rlines = new ArrayList<>();
		Iterable<String> ite_select = Iterables.limit(
				Iterables.skip(lines, start_included), end_excluded
						- start_included);
	 ...
	 * @return
	 */
	public static String selectAsString(Lines lines, int start_included,
			int end_excluded) {
		StringBuilder sBuilder = new StringBuilder();
		Iterable<String> ite_select = Iterables.limit(
				Iterables.skip(lines, start_included), end_excluded
	 ...
	 * @return
	 */
	public static Lines selectAsLines(Lines lines, int start_included,
			int end_excluded) {
		ArrayList<String> rlines = new ArrayList<>();
		Iterable<String> ite_select = Iterables.limit(
				Iterables.skip(lines, start_included), end_excluded
	...
	public static LineSequenceWriter getLineWriter(String fileName,
			boolean compressed) throws Exception {
		return new LineSequenceWriter(fileName, compressed, Charsets.UTF_8);
	}
	
	public static LineSequenceWriter getLineWriter(String fileName,
			boolean compressed, Charset charset) throws Exception {
		return new LineSequenceWriter(fileName, compressed, charset);
	}
	...
	public static boolean flushToFile(String str, String fileToSave,
			Charset charset) {
		fileToSave = DmasApplication.applyDataContext(fileToSave);
