...
public class Model {
	...
	protected boolean readTAssignFile(String tassignFile) {
		try {
			...
			for (i = 0; i < M; i++) {
				...
				LDADocument doc = new LDADocument(words);
				data.setDoc(doc, i);
	 ...
	 * load d model
	 */
	public boolean loadModel(String dir) {
	 ...
	 * Save model
	 */
	public boolean saveModel(String dir, String modelName) {

		if (!data.localDict.writeWordMap(dir + File.separator + wordMapFile)) {
			return false;
		}
	 ...
	 */
	public boolean initNewModelForEstimation(Iterable<Document> trainData) {

		this.data = LDADataset.convertDocument(trainData);
	 ...
	 */
	public boolean initNewModelForInference(Iterable<Document> newData, Model trnModel) {
		...
		data = LDADataset.convertDocument(newData, trnModel.data.localDict);
	...
	public Map<String, double[]> loadRepresentation() {
		ArrayList<String> idMap = this.data.docIndIdMap;
		return IntStream.range(0, idMap.size()).mapToObj(ind -> ind)
				.collect(Collectors.toMap(ind -> idMap.get(ind), ind -> this.theta[ind]));
	}
