...
public class PAN14TestPos2Vec {
	...
	public static TestEntry<S2VParam> test(PAN2014AV2 ds, S2VParam param, String testCase, String cacheFile,
		...
		TestEntry<S2VParam> entry = TestEntries.retrieve(cacheFile, S2VParam.class, param);
		if (!test && entry != null)
			return entry;
		...
		try {
			...
			if (!test)
				TestEntries.update(cacheFile, S2VParam.class, ent);
			return ent;
		} catch (Exception e) {
		...
		pool.submit(() -> {
			ds.trainingSet.keySet().parallelStream().forEach(testcase -> {
				...
				if (last_max_param != null) {
					...
					TestEntry<S2VParam> max = test(ds, last_max_param, testcase, resultCatchFile, true);
