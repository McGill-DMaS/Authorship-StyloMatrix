...
public class ContextGeneratorReordering extends ContextGenerator {

	public ContextGeneratorReordering(Parser parser) {
		this.parser = parser;
	}
	...
	public Iterable<Context> generateContext(Sentence sentence,
		...
		tree = sentence.tree;
		this.wMap = wMap;
		return new ContextTREEIterable();
	}
			...
			private class ContextTREETD extends Context {
				...
				public List<Word> getBaseContext() {
					...
					List<List<Word>> right = current.subList(l_index + 1,
							current.size());
					return new MergedList<Word>(true, new MergedList<>(true, left, right));
				}

				@Override
				public List<Word> getPredictionContext() {
					return new MergedList<>(true,current.subList(l_index, l_index + 1));
				}
