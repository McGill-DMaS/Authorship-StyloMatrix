...
public class Inferencer {
	...
	public Inferencer(String estimatedModelDir, Random rand) {
		this.rand = rand;
		trnModel = new Model(rand);
		trnModel.initEstimatedModel(estimatedModelDir);
		computeTrnTheta();
		computeTrnPhi();
	}

	public Inferencer(Model trnModel, Random rand) {
		this.rand = rand;
	...
	public Map<String, double[]> inferNewDocs(Iterable<Document> docs) {
		return this.inference(docs).loadRepresentation();
	}

	// inference new model ~ getting data from a specified d ataset
	public Model inference(Iterable<Document> newData) {
		Model newModel = new Model(rand);
		newModel.initNewModelForInference(newData, trnModel);
